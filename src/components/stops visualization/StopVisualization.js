...
class StopVisualization extends Component {
	render() {
		...
		const dataPoints = { points: `${createDataHandler(this.props.stops)}` };
		const secondDataPoints = this.props.activeRoute
			? { points: `${createDataHandler(this.props.activeRoute)}` }
			: null;
		...
		const activeVisible = (
			<g className="data" data-setname="Our first data set">
				...
				{this.props.activeRoute
					? this.props.activeRoute.map(stop => {
							return (
								<g>
		...
		const visualize = (
			...
			>
				...
				{this.props.renderActive ? activeVisible : null}
			</svg>
		);
...
export default connect(mapStateToProps)(StopVisualization);
